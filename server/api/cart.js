...
router.put('/:orderId/shipping', async (req, res, next) => {
  try {
    const orderId = req.params.orderId;
    ...
    order.shipTo = shippingInfo.shipTo;
    order.address = shippingInfo.streetAddress;
    order.city = shippingInfo.city;
    order.state = shippingInfo.state;
    order.zipCode = shippingInfo.zipCode;
